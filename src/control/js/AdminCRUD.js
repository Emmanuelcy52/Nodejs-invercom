      ...
      .then((data) => {
        ...
        if (!data.error) {
          if (!data.Admins || data.Admins.length === 0) {
            console.log("valla no se como fuiste capas de hacer eso pero felicidades lo acabas de romper :,(");
          } else {
            data.Admins.forEach((Admins) => {
              ...
              filaDatos.innerHTML = `
                <td>${Admins.nombre_administrador}</td>
                <td>${Admins.nombre_real}</td>
                <td">
                    <a class="btn btn-primary" href="/?id=${Admins.id}">Editar</a>
                  <button class="btn btn-danger" onclick="eliminarDatos(${Admins.id})">Eliminar</button>
                </td>
              `;
